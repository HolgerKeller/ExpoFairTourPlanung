...
AS
BEGIN
 select DeliveryType,JobType, Address, count(Address) Count from [expofair].[job2Tour] where cast(JobDate as Date) = convert(date, @EventDate) and ( IdTour is Null or IdTour = 0 ) and JobType <> '.Bessemerstraße' group by DeliveryType,Jobtype, Address order by DeliveryType,Jobtype, Address
END
GO
